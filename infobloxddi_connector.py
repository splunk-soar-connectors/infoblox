...
class InfobloxddiConnector(BaseConnector):
    ...
    def _is_ip(self, cidr_ip_address):
        ...
        try:
            ...
            self.debug_print(f"{consts.INFOBLOX_IP_VALIDATION_FAILED}. {error_message}")
            return False
    ...
    def _get_rp_zone_details(self, action_result, zone_filter_params):
        ...
        zone_filter_params.update({consts.INFOBLOX_JSON_RETURN_FIELDS: consts.INFOBLOX_LIST_RP_ZONE_PARAMS})

        # Getting rp_zone details
        rp_zone_details_status, rpz_zone_details = self._make_paged_rest_call(
            consts.INFOBLOX_RP_ZONE_DETAILS_ENDPOINT, action_result, params=zone_filter_params, method="get"
        )
    ...
    def _validate_rp_zone(self, action_result, zone_details_param):
        ...
        if not rp_zone_details:
            self.debug_print(consts.INFOBLOX_RP_ZONE_NOT_EXISTS.format(fqdn_name=zone_details_param[consts.INFOBLOX_PARAM_FQDN]))
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(
                phantom.APP_ERROR, consts.INFOBLOX_RP_ZONE_NOT_EXISTS.format(fqdn_name=zone_details_param[consts.INFOBLOX_PARAM_FQDN])
            )
        ...
        if rp_zone_details[consts.INFOBLOX_RPZ_POLICY] != consts.INFOBLOX_BLOCK_POLICY_RULE:
            self.debug_print(consts.INFOBLOX_RP_ZONE_POLICY_RULE_ERROR.format(rule_name=rp_zone_details[consts.INFOBLOX_RPZ_POLICY]))
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(
                phantom.APP_ERROR, consts.INFOBLOX_RP_ZONE_POLICY_RULE_ERROR.format(rule_name=rp_zone_details[consts.INFOBLOX_RPZ_POLICY])
            )
    ...
    def _make_rest_call(self, endpoint, action_result, params=None, data=None, method="post", timeout=None):
        ...
        try:
            ...
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(phantom.APP_ERROR, consts.INFOBLOX_ERROR_API_UNSUPPORTED_METHOD.format(method=method)), response_data
        except Exception as e:
            error_message = self._get_error_message_from_exception(e)
            self.debug_print(f"{consts.INFOBLOX_EXCEPTION_OCCURRED}. {error_message}")
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(phantom.APP_ERROR, f"{consts.INFOBLOX_EXCEPTION_OCCURRED}. {error_message}"), response_data

        # Make the call
        try:
            if timeout is not None:
                request_obj = request_func(
                    f"{self._url}{consts.INFOBLOX_BASE_ENDPOINT}{endpoint}",
                    auth=credential_data,
                    params=params,
                    data=data,
                    timeout=timeout,
            ...
            error_message = self._get_error_message_from_exception(e)
            self.debug_print(f"{consts.INFOBLOX_ERROR_SERVER_CONNECTION}. {error_message}")
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(phantom.APP_ERROR, f"{consts.INFOBLOX_ERROR_SERVER_CONNECTION}. {error_message}"), response_data

        # Handling the 404 status code for list_rpz action
        if self.get_action_identifier() == "list_rpz" and request_obj.status_code == consts.INFOBLOX_REST_RESP_NOT_FOUND:
            response_data = {consts.INFOBLOX_RESOURCE_NOT_FOUND: True}
            try:
        ...
        if request_obj.status_code in ERROR_RESPONSE_DICT:
            ...
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(
                phantom.APP_ERROR, consts.INFOBLOX_ERROR_FROM_SERVER.format(status=request_obj.status_code, detail=message)
            ), response_data
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            self.debug_print("{}. {}".format(message, error_message))
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(phantom.APP_ERROR, "{}. {}".format(message, error_message)), response_data
    ...
    def _get_system_info(self, param):
        ...
        params = {consts.INFOBLOX_JSON_RETURN_FIELDS: consts.INFOBLOX_LEASE_RETURN_FIELDS}
        ...
        # Optional parameter
        params[consts.INFOBLOX_JSON_NETWORK_VIEW] = param.get(consts.INFOBLOX_JSON_NETWORK_VIEW, consts.INFOBLOX_NETWORK_VIEW_DEFAULT)
        ...
        if param.get(consts.INFOBLOX_JSON_NETWORK_VIEW, consts.INFOBLOX_NETWORK_VIEW_DEFAULT) == consts.INFOBLOX_NETWORK_VIEW_DEFAULT:
            ...
            if phantom.is_ip(ip_hostname):
                ...
                host_status, host_response = self._make_rest_call(
                    consts.INFOBLOX_RECORDS_IPv4_ENDPOINT, action_result, params=record_a_params, method="get"
                )
            ...
            # Invoking record:a if ipv6
                ...
                # Make call to get lease information
                host_status, host_response = self._make_rest_call(
                    consts.INFOBLOX_RECORDS_IPv6_ENDPOINT, action_result, params=record_aaaa_params, method="get"
                )
        ...
        if not lease_response_list and host_response_list:
            for host_data in host_response_list:
                ...
                data[consts.INFOBLOX_JSON_ADDRESS] = host_data.get(consts.INFOBLOX_JSON_A_IP, host_data.get(consts.INFOBLOX_JSON_AAAA_IP))
        ...
        for data in lease_response_list:
            ...
            data[consts.INFOBLOX_JSON_CLTT] = time.strftime(consts.INFOBLOX_JSON_DATE_FORMAT, time.localtime(data[consts.INFOBLOX_JSON_CLTT]))
            data[consts.INFOBLOX_JSON_STARTS] = time.strftime(
                consts.INFOBLOX_JSON_DATE_FORMAT, time.localtime(data[consts.INFOBLOX_JSON_STARTS])
            )
            data[consts.INFOBLOX_JSON_ENDS] = time.strftime(consts.INFOBLOX_JSON_DATE_FORMAT, time.localtime(data[consts.INFOBLOX_JSON_ENDS]))
    ...
    def _list_network_view(self, param):
        ...
        status, response = self._make_paged_rest_call(consts.INFOBLOX_NETWORK_VIEW, action_result, method="get")
    ...
    def _block_domain(self, param):
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Please provide a valid 'domain' parameter. {error_message}")
        ...
        rpz_rule_name = f"{domain_name}.{rp_zone}"
        ...
        check_name_details_status, rpz_rule_name_details = self._make_rest_call(
            consts.INFOBLOX_DOMAIN_ENDPOINT, action_result, params=check_rpz_rule_params, method="get"
        )
        ...
        api_data = {
            consts.INFOBLOX_RPZ_RULE_NAME: rpz_rule_name,
            consts.INFOBLOX_JSON_RP_ZONE: rp_zone,
            consts.INFOBLOX_PARAM_CANONICAL: "",
            consts.INFOBLOX_PARAM_VIEW: view,
        }
    ...
    def _block_ip(self, param):
        ...
        rpz_rule_name = f"{ip_address.lower()}.{rp_zone}"
        ...
        check_name_details_status, rpz_rule_name_details = self._make_rest_call(
            consts.INFOBLOX_IP_ENDPOINT, action_result, params=check_rpz_rule_params, method="get"
        )
    ...
    def _unblock_ip(self, param):
        ...
        unblock_status, unblock_response = self._make_rest_call(f"/{ref}", action_result, method="delete")
    ...
    def _list_rpz(self, param):
        ...
        zone_details_param = {consts.INFOBLOX_PARAM_VIEW: param.get(consts.INFOBLOX_JSON_NETWORK_VIEW, consts.INFOBLOX_NETWORK_VIEW_DEFAULT)}
    ...
    def _list_hosts(self, param):
        ...
        ipv4_hosts_status, ipv4_hosts = self._make_paged_rest_call(
            consts.INFOBLOX_RECORDS_IPv4_ENDPOINT, action_result, params=ipv4_hosts_param, method="get"
        )
        ...
        ipv6_hosts_status, ipv6_hosts = self._make_paged_rest_call(
            consts.INFOBLOX_RECORDS_IPv6_ENDPOINT, action_result, params=ipv6_hosts_param, method="get"
        )
...
def main():
    ...
    if username and password:
        try:
            login_url = f"{InfobloxddiConnector._get_phantom_base_url()}/login"
